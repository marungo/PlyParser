...
The first argument is a one-character string that defines the key
    you want to bind
...
The second argument is a function object. The system will invoke
    ...
    name of a function, like fred, you will omit the parens,
    because you are not invoking fred now, the system will be
    invoking it later.
      
    
The third argument is the documentation string that is displayed
    if the user presses the question mark key (?) to show the
    available keyboard callbacks.
    
The optional fourth argument is whether to set this callback in a
...
*/
TW.onkeypress = function (e,canvas) {
...
TW.mainInit = function(renderer,scene,inits) {
     ...
     * a parent element by putting {parent: } in the inits
...
TW.createWireBarn = function(width, height, length) {
    ...
    */
    ...
    var mat = new THREE.LineBasicMaterial( {color: 0xffffff, vertexColors: THREE.FaceColors});

    var barnObj = new THREE.Line( geometry, mat, THREE.LinePieces );
...
document.addEventListener('keypress',
                          ...
                          function(event) {
                              ...
                              if(TW.lastClickTarget)

                                  TW.onkeypress(event,TW.lastClickTarget);
...
TW.createUSFlagGray = function (log2size) {
    ...
    // console.log("after union\n",TW.hexString(data,width,height));
...
TW.createUSFlagRedWhiteBlue = function (log2size) {
    ...
    for( i = 0 ; i < 13 ; i+=2 ) doStripe( i, 0xFF, 0, 0 );

    // console.log("doing 6 white stripes");

    for( i = 1 ; i < 13 ; i+=2 ) doStripe( i, 0xFF, 0xFF, 0xFF );
    ...
    for( i = 0 ; i < unionHoist; i++ )
        ...
        for( j = 0 ; j < unionFly; j++ )
            ...
            TW.rowMajorAsetRGB(data,i,j, 0,0,0xFF);
...
Date Modified:
 ...
 Creative Commons License  Viewable With Any Browser  Valid HTML 4.01 Strict  Valid CSS!